...
if ( ! class_exists( 'KTPWP_Graph_Renderer' ) ) {
	...
	class KTPWP_Graph_Renderer {
		...
		public function render_dummy_graph() {
			...
			$upgrade_url = 'https://www.kantanpro.com/downloads/kantanpro-download-2';
			...
			$content .= '<h3 style="margin:50px 0 15px;color:#333;font-size:24px;text-shadow:0 1px 2px rgba(255,255,255,0.8);">' . esc_html__( 'レポート機能を利用するには', 'ktpwp' ) . '</h3>';
			$content .= '<a href="' . $upgrade_url . '" target="_blank" style="display:inline-block;background:linear-gradient(135deg, #e74c3c, #c0392b);color:#fff;font-size:16px;font-weight:bold;padding:15px 32px;margin-top:10px;border-radius:50px;text-decoration:none;box-shadow:0 4px 15px rgba(231,76,60,0.3);transition:transform 0.3s, box-shadow 0.3s;border:none;text-transform:uppercase;letter-spacing:1px;" onmouseover="this.style.transform=\'translateY(-3px)\';this.style.boxShadow=\'0 8px 20px rgba(231,76,60,0.4)\';" onmouseout="this.style.transform=\'translateY(0)\';this.style.boxShadow=\'0 4px 15px rgba(231,76,60,0.3)\';">' . esc_html__( 'アップグレード', 'ktpwp' ) . '</a>';
			$content .= '<p style="margin-top:15px;color:#555;font-size:14px;">' . esc_html__( 'KantanPro(a)バージョンにアップグレードすると、レポート機能が利用できるようになります', 'ktpwp' ) . '</p>';
			$content .= '</div></div>';
